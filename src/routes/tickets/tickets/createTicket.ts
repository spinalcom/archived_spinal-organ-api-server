  ...
  */
  app.post("/api/v1/ticket/create_ticket", async (req, res, next) => {
    try {
      ...
      const [node, workflowById, processById]: SpinalNode<any>[] = await _load(arrayofServerId)
      //@ts-ignore
      ...
      if (workflowById === undefined && processById === undefined) {
        ...
        if (workflowByName) {
          var allProcess = await serviceTicketPersonalized.getAllProcess(workflowByName.getId().get())
        ...
        if (processByName.belongsToContext(workflowByName)) {
          ticketCreated = await serviceTicketPersonalized.addTicket(ticketInfo, processByName.getId().get(), workflowByName.getId().get(), node.getId().get())
        }
        else {
        ...
        if (processById) {
          if (processById.belongsToContext(workflowById)) {
            ticketCreated = await serviceTicketPersonalized.addTicket(ticketInfo, processById.getId().get(), workflowById.getId().get(), node.getId().get())
          } else {
            res.status(400).send("the workflow does not contain this process");
          }
        }
