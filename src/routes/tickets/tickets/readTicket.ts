...
*/
  app.get("/api/v1/ticket/:ticketId/read_details", async (req, res, next) => {
    try {
      var _ticket: SpinalNode<any> = await spinalAPIMiddleware.load(parseInt(req.params.ticketId, 10));
      //@ts-ignore
      SpinalGraphService._addNode(_ticket)
      //Step
      var _step = await _ticket.getParents("SpinalSystemServiceTicketHasTicket").then((steps) => {
      ...
      var _process = await _step.getParents("SpinalSystemServiceTicketHasStep").then((processes) => {
      ...
      //Context
      var contextRealNode = SpinalGraphService.getRealNode((_ticket.getContextIds())[0])
      ...
      // Files
      var files = await getFiles(_ticket)
      var _files = []
      ...
      try {
        if (_ticket.info.elementSelected !== undefined)
          elementSelected = await spinalAPIMiddleware.loadPtr(_ticket.info.elementSelected)
        else
          elementSelected = SpinalGraphService.getRealNode(_ticket.info.nodeId.get());
      } catch (error) {
        console.error(error);
      }
