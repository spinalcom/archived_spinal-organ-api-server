  ...
  app.get("/api/v1/analytics/room/:id/status/:option", async (req, res, next) => {
    try {
      ...
      if (room.getType().get() === "geographicRoom") {
        ...
        var ticketListRoom = await serviceTicketPersonalized.getTicketsFromNode(room.getId().get());
        for (let index = 0; index < ticketListRoom.length; index++) {
          var realNodeTicket = SpinalGraphService.getRealNode(ticketListRoom[index].id)
          ...
          //Context
          var contextRealNode = SpinalGraphService.getRealNode((realNodeTicket.getContextIds())[0])
          ...
          var infoTicket = {
            ...
            description: realNodeTicket.info.description === undefined ? "" : realNodeTicket.info.description.get(),
        ...
        for (const equipement of equipements) {
          ...
          var ticketListEquipemnt = await serviceTicketPersonalized.getTicketsFromNode(equipement.getId().get());


          for (const ticketEquipemnt of ticketListEquipemnt) {
            var realNodeEquipementTicket = SpinalGraphService.getRealNode(ticketEquipemnt.id)


            //Step
            var _stepTicketEquipement = await realNodeEquipementTicket.getParents("SpinalSystemServiceTicketHasTicket").then((steps) => {
              for (const step of steps) {
                if (step.getType().get() === "SpinalSystemServiceTicketTypeStep") {
                  return step
                }
            ...
            var logs = await serviceTicketPersonalized.getLogs(realNodeEquipementTicket.getId().get())
            for (const log of logs) {
              let lastActionDate = log.creationDate
            ...
            //Context
            var contextRealNodeEquipementTicket = SpinalGraphService.getRealNode((realNodeEquipementTicket.getContextIds())[0])
            ...
            var infoTicketEquipement = {
              ...
              description: realNodeEquipementTicket.info.description === undefined ? "" : realNodeEquipementTicket.info.description.get(),
        ...
        for (const objectroom of ticketList) {
          ...
          if (objectroom.priority === 0 || objectroom.priority === "0") Occasionally++
          else if (objectroom.priority === 1 || objectroom.priority === "1") Normal++
          else if (objectroom.priority === 2 || objectroom.priority === "2") Urgent++
        }
        for (const objectEquipement of _ticketListEquipemnt) {
          allTicket.push(objectEquipement)
          if (objectEquipement.priority === 0 || objectEquipement.priority === "0") Occasionally++
          else if (objectEquipement.priority === 1 || objectEquipement.priority === "1") Normal++
          else if (objectEquipement.priority === 2 || objectEquipement.priority === "2") Urgent++
        }
