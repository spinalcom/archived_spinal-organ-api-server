...
module.exports = function (logger, app, spinalAPIMiddleware) {
  ...
  */
    app.delete("/api/v1/device/:id/delete", (req, res, next) => __awaiter(this, void 0, void 0, function* () {
        try {
            const profileId = (0, requestUtilities_1.getProfileId)(req);
            let device = yield spinalAPIMiddleware.load(parseInt(req.params.id, 10), profileId);
            // @ts-ignore
            spinal_env_viewer_graph_service_1.SpinalGraphService._addNode(device);
            if (device.getType().get() === "BmsDevice") {
                device.removeFromGraph();
            }
            else {
